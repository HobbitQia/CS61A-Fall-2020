...
class Ant(Insect):
    ...
    def __init__(self, armor=1):
        ...
        self.is_doubled = False # to mark whether its damage was doubled by the queen
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # Target:self should refer to the container
            old_ant = place.ant
            new_ant = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while checking_place is not beehive:
            if len(checking_place.bees) > 0 and self.min_range <= moves <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            amount += self.damage
        bees = now_at.bees[:]  # Create a new list
        for bee in bees:
...
class ScubaThrower(ThrowerAnt):
    """"Another kind of Throwernt which is watersafe.
    """
...
# END Problem EC
    ...
    # BEGIN Problem EC
    implemented = True   # Change to True to view in the GUI
    true_queen = False # Is there a true queen?
    # END Problem EC
