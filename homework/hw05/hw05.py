...
class VendingMachine:
    ...
    def __init__(self, product, price):
        ...
        self.stock = 0
        self.price = price
        self.fund = 0 # money that customers have added
    ...
    def add_funds(self, added_fund):
        if self.stock == 0:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(added_fund)
        else:
            self.fund += added_fund
...
def is_bst(t):
    ...
    if t == [] or t.is_leaf():
        return True
    if not isinstance(t, Tree) or len(t.branches) > 2:
        return False
    left_branch, left_val= t.branches[0], bst_max(t.branches[0])
    if len(t.branches) == 1:
        return is_bst(left_branch)
    right_branch, right_val = t.branches[1], bst_min(t.branches[1])
    if not is_bst(left_branch) or not is_bst(right_branch):
        return False
    elif left_val > t.label or right_val <= t.label:
        return False
    else:
        return True
...
def bst_min(t):
    if t.is_leaf():
        ...
        if len(t.branches) == 2:
            ...
            return min(t.label, left_min,right_min)
        else:
...
def bst_max(t):
    if t.is_leaf():
        ...
        if len(t.branches) == 2:
            ...
            return max(t.label, left_max,right_max)
        else:
